...
An application that fills only some voids of its head is a \emph{partial
  application}: its result is a fresh formation that retains the
  unfilled voids and closes the filled ones.
The reduction is part of the \(\varphi\)-calculus and is not duplicated
  here; \citet{bugayenko2021eolang} gives the formal rule.
...
Data literals (\ff{STRING}, \ff{TEXT}, \ff{INT}, \ff{FLOAT}, \ff{HEX},
  ...
  byte sequence of the literal value, decorated by the appropriate
  canonical object
  ($\Phi.\mathtt{number}$, $\Phi.\mathtt{string}$, $\Phi.\mathtt{bytes}$),
  following the encoding of \citet{bugayenko2021eolang}.
The parser produces the $\Delta$-asset whose bytes are the lexical
  value of the literal; the surrounding canonical-object decoration is
  determined by the literal's type.

\paragraph{Tuples.}
The canonical object $\Phi.\mathtt{tuple}$ represents a finite, ordered
  sequence of objects.
A tuple value $\Phi.\mathtt{tuple}(e_1, \ldots, e_n)$ is a copy of the
  canonical $\mathtt{tuple}$ formation with $n$ positional attributes
  filled, indexed by $\alpha_0, \ldots, \alpha_{n-1}$.
The standard library provides attributes such as $\mathtt{at}$ (indexed
  access) and $\mathtt{length}$ on $\Phi.\mathtt{tuple}$, but these are
  ordinary canonical attributes and are not part of the language proper.
Tuples are used to represent the trailing-arguments form of compact
  tuple lines (\cref{sec:sem:tuple}) and the result of a bare \ff{*}
  application head.
