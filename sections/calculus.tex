...
The \ff{color} attribute is attached to another object formation with one \(\Delta\)-\emph{asset}, which is attached to \emph{data} (three bytes).
...
Attributes are \emph{immutable}, i.e. an application of a binding to an object, where the attribute is already attached, results in a \emph{terminator} denoted as \(\stx{\dead}\) (runtime error), for example:
\begin{phiquation*}
...
is either \(\stx{\dead}\) or object formation without \(\lambda\) asset.
\end{definition}
...
Here, if the parent of \(\Phi.\ff{x}.\ff{next}\) were attached in the formation, the result of \(\mathbb{D}(\Phi.\ff{k}.\ff{next})\)
  would not be equal to \ff{43}. Instead, it would be equal to \(\stx{\dead}\), because \(\Phi.\ff{k}.\ff{next}.\rho\)
...
is called, the \ff{this} pointer refers to the object that owns the method.
