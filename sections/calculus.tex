...
In this Section we introduce \phic{}, a formalism that we use later in order to optimize object-oriented code.
...
alphabet includes three non-terminals that rewrite to terminals as such:
\begin{itemize}
...
Since any expression may recursively be defined as either \begin{inparaenum}[1)]
    ...
    \item dispatch,
\end{inparaenum}
it consists of a \textbf{head} and a possibly empty \textbf{tail}, together denoted as \(h\bullet{}t\).
\end{definition}
...
The \textbf{terminator}, denoted as \stx{\dead}, is an object that equals itself when being
a subject of a dispatch or an application:
\begin{equation*}
...
by attaching it to ``\stx{\varphi}''-attribute of another object (``decorator''),
...
that if there are reduction sequences from any expression to two different expression,
