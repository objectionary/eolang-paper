...
The \ff{color} attribute is attached to another object formation with one \stx{\Delta}-\emph{asset},
...
\end{phiquation}
The leftmost symbol \stx{\xi} denotes the \emph{scope} of the object, which is the object formation itself;
  the following \(\stx{.}\ff{p}\) part retrieves the object attached to the attribute \(\ff{p}\);
  then, the application $(|t|->b)$ makes a copy of the object;
  finally, the \(\stx{.}\ff{y}\) part retrieves object \(b\).
...
Attributes are \emph{immutable}, i.e. an application of a binding to an object,
  ...
  results in a \emph{terminator} denoted as \stx{\dead} (runtime error), for example:
\begin{phiquation*}
...
An object is called a \emph{decorator} if it has the \stx{\varphi}-attribute
...
An object may have data attached only to its \stx{\Delta}-asset, for example:
\begin{phiquation*}
[[ |x| -> [[ D> 00-2A ]] ]].
\end{phiquation*}

An object may have a \emph{function} attached to its \stx{\lambda}-asset.
...
and then either returns the data attached to the \stx{\Delta}-asset of
...
A program is an object that is attached to the \stx{\Phi} attribute
of the \emph{Universe}, for example:
\begin{phiquation*}
Q -> [[ D> CA-FE ]].
\end{phiquation*}
The dataization of \stx{\Phi} is the evaluation of the program.
...
A \textbf{program} is a pair $ Q \stx{ -> } \stx{ [[ } B \stx{ ]] } $ that exists only
...
\textbf{Atom} is an object with a function attached to its \stx{\lambda}-asset.
\end{definition}
...
e\stx{.}\tau \stx{(}\rho \stx{->} e_2 \stx{)} \equiv e\stx{.}\tau \
\quad\text{(by \nameref{r:dot} and \nameref{r:stay})}
...
{ \langle \stx{ [[ } B_1 , \stx{ L> } f , B_2 \stx{ ]] } \bullet{} t, s_1 \rangle \stepto \langle p, s_2 \rangle }  \
\;\text{if}\; f( \stx{ [[ } B_1, B_2 \stx{ ]] }, s_1 ) \to \langle e, s_2 \rangle

\newrule[\Phi]{Mphi} \
\frac \
{ \langle e \bullet{} t, s_1 \rangle \stepto \langle p, s_2 \rangle} \
{ \langle \stx{ \Phi } \stx{ . } \tau \bullet{} t, s_1 \rangle \stepto \langle p, s_2 \rangle }  \
\;\text{if}\; \stx{ \Phi } \stx{ \mapsto } \stx{ \llbracket } B_1 , \tau \stx{ \mapsto } e, B_2 \rrbracket
\end{phiquation*}
\end{mdframed}
\capt{Morphing rules.}
\label{fig:morphing}
...
\frac \
{ } \
{ \langle \stx{ [[ } B_1, \stx{ D> } \delta, B_2 \stx{ ]] }, s \rangle \stepto \langle \delta, s \rangle } \
\quad\quad \
\newrule{norm} \
...
{ \langle \stx{ [[ } B_1, \stx{ @ } \stx{ -> } e, B_2 \stx{ ]] }, s_1 \rangle  \stepto \langle \delta, s_2 \rangle } \
...
% \begin{table*}
% \capt{A few examples of objects and their \(\Delta\)-depths.}
% \label{tab:depths}
% \begin{tabular}{lll}
% \toprule
