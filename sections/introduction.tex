...
The semantics is defined by a set of reduction rules that are proven to be confluent,
  ensuring that evaluation order does not affect the final result.

The contributions of this paper are as follows:
\begin{itemize}
  \item A formal syntax for \phic{} defined by a context-free grammar (\cref{sec:syntax}).
  \item Rigorous definitions of the core constructs: expressions, formations, attributes, and bindings (\cref{sec:foundations}).
  \item A family of semantic operators including contextualization, normalization, morphing, and dataization (\cref{sec:operators}).
  \item A proof that the reduction strategy is confluent (\cref{sec:confluence}).
\end{itemize}
...
\Cref{sec:confluence} proves the confluence of the reduction rules.
