...
Formation(attributes):
  take(attr):
    if attributes.absent(attr): return (*@\(\dead\)@*)
    c := attributes.get(attr)
    if c is (*@\(\varnothing\)@*): return (*@\(\dead\)@*)
    return c
  put(attr, cell):
    if attributes.absent(attr): return (*@\(\dead\)@*)
    c := attributes.get(attr)
    if c is not (*@\(\varnothing\)@*): return (*@\(\dead\)@*)
...
Decoration(decorator, decoratee, memos):
  take(attr):
    if memos.absent(attr):
      c := decorator.take(attr)
      if c is (*@\(\dead\)@*):
        c := decoratee.take(attr)
      memos.put(attr, c)
