...
\begin{inparaenum}[1)]
\item an associative array of attributes and actor references,
...
If the \(\lambda\)-asset is also absent by the \(\varphi\)-attribute is present, the actor asks the actor referenced by the attribute to \ff{take()}.
Otherwise, the actor returns an error.
