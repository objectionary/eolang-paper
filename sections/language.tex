...
\iexec{phino rewrite --sweet --flat --nonumber --output=latex --input=xmir --hide=Q.order.xi🌵 < .eoc/1-parse/order.xmir}

\cref{fig:order} shows eight cells that represent the \ff{order} object at runtime.
...
\node [label={[up, text width=10em]$c_2=[[ qty -> ?, \newline price -> c_7, \rho -> ? ]]$}, label={[cell]formation}, below=1.2 of c1, fill=blue!5] (c2)
  ...
  \(\rho\): & \(\varnothing\) \\
  \end{tabularx}}};
\node [label={[up]$c_3 = c_0.qty$}, label={[cell]dispatch}, right=of c0, fill=green!5] (c3)
  {\parbox{7em}{
  \begin{tabularx}{\linewidth}{Xl}
  \ff{base}: & \(c_2\) \\
  \ff{attr}: & \ff{qty} \\
...
\node [label={[up]$c_4 = c_3.mul$}, label={[cell]dispatch}, fill=green!5, right=of c3] (c4)
  ...
  \ff{memo}: & \(\varnothing\) \\
  \end{tabularx}}};
\node [label={[up]$c_6 = c_0.price$}, label={[cell]dispatch}, fill=green!5, right=of c1] (c6)
  {\parbox{8em}{
  \begin{tabularx}{\linewidth}{Xl}
  \ff{base}: & \(c_2\) \\
  \ff{attr}: & \ff{price} \\
...
The code of \cref{fig:order} may be transformed to a SODG using the following sequence of instructions:

\begin{ffcode}
atom(c1, "qty", "price", "cost")
...
A function may not delete cell from the SODG.
Instead, they get deleted automatically.
All cell created by a function are deleted, except the cell that the function returns.
The cells that are directly or indirectly referenced by the returned cell are not deleted either.
