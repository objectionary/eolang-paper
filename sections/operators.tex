...
{ \langle  \Phi   .  \tau \bullet{} t, u, s_1 \rangle \stepto \langle p, s_2 \rangle }  \
\;\text{if}\; u =  \llbracket  B_1 , \tau  \mapsto  e, B_2 \rrbracket
\end{phiquation*}
\end{mdframed}
\capt{Morphing rules.}
\label{fig:morphing}
...
The notation \(\mathbb{D}(e, u)\) is a shortened form of $\mathbb{D}(e, u, ?)_{(1)}$,
...
\begin{lemma}
The dataization function is partial, because not every primitive contains \phiTerminal{\(\Delta\)}-asset.
\end{lemma}
...
\nf \cong  [[   @   ->  \nf  ]]  \quad \text{(by \nameref{r:phi})}
 ...
 [[  B  ]]  \cong  [[  B  ]] .@  \quad\text{if}\;  @  \in B \;\text{and}\;  \Delta  \not\in B \quad \text{(by \nameref{r:phi} and \nameref{r:dot})}
