...
Assets $D$ and $L$ do not belong to the object domain.
...
A \textbf{dispatch}, denoted as $ e . \tau $, where \(e\) is the ``subject'',
  means retrieval of expression from \(e\) by the attribute \(\tau\).
\end{definition}
...
its already attached attribute equals $T$:
\begin{phiquation*}
...
An \textbf{atom} is an object with a function attached to its $L$-asset.
\end{definition}
...
A \textbf{primitive}, denoted by \(p\) and ranging over \(\mathcal{P} \in \mathcal{N}\),
  is either $T$ or object formation without $L$-asset.
\end{definition}
