...
An \textbf{asset} is an identifier to which either data
  (denoted as \phiTerminal{\(\Delta\)}-asset) or function
  (denoted as \phiTerminal{\(\lambda\)}-asset) is attached.
\end{definition}
...
Assets \phiTerminal{\(\Delta\)} and \phiTerminal{\(\lambda\)} do not belong to object domain.
...
The arrow $ -> $ denotes an attachment of an expression (right-hand side)
  to an attribute (left-hand side). The arrow $ ..> $ denotes
  an attachment of data or function to an asset.

\subsection{Program}\label{sec:program}

\begin{definition}[Program]
A \textbf{program} is a pair $ Q -> [[ B ]] $ that exists outside of any objects.
\end{definition}

The object attached to $Q$ is called \emph{Universe}.
...
Object \textbf{application}, denoted as $ b ( \tau -> e ) $,
  is a copy of an existing abstract object \(b\) (the ``subject''), with \(e\) attached to its \(\tau\) attribute.
\end{definition}
...
Even though $?$ may be attached to an attribute of an object,
it is not an object by itself. Instead, $?$ is a ``placeholder''
...
Object \textbf{dispatch} (also known as ``dot notation''), denoted as
  $ b . \tau $ where \(b\) is the ``subject'', means retrieval of the object
...
In an expression, the locator $Q$ means the program,
while the locator \phiTerminal{\(\xi\)} means the scope of the expression.
...
The \textbf{terminator}, denoted as $T$, is an object that equals itself when it is
...
its already attached attribute equals to $T$:
\begin{equation*}
...
An \textbf{atom} is an object with a function attached to its \phiTerminal{\(\lambda\)}-asset.
\end{definition}

\Cref{eq:Sqrt} demonstrates an atom with a function that calculates the square root of a number,
  which it retrieves from the \phiTerminal{\(\Delta\)}-asset
...
Object \textbf{decoration} is a mechanism of extending an object (``decoratee'')
  by attaching it to the $@$-attribute of another object (``decorator''),
...
Attaching expression \(e\) to the $^$ attribute of object \(b\)
...
A \textbf{primitive} denoted by \(p\) and ranging over \(\mathcal{P} \in \mathcal{N}\)
  is either $T$ or object formation without \phiTerminal{\(\lambda\)}-asset.
\end{definition}
