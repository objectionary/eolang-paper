...
A \textbf{domain} of formation \(b\), denoted as \(\bar{b}\), is an ordered set of all attributes of \(b\).
\end{definition}
...
\Cref{eq:price-color} is an example of a formation that is not an abstract,
  while the expression attached to its |color| attribute is an abstract.
...
An attribute's \textbf{ordinal}, denoted as \(\ordinal{\tau}{b}\),
  ...
  in \(\bar{b}\), starting from zero, not counting assets.
\end{definition}
