...
A \textbf{state} of evaluation \(s_i\) ranges over \(\mathcal{S}\),
  where \(\varnothing_s\) is an empty state.
\end{definition}
...
\(\langle \mathcal{B}, \mathcal{B}, \mathcal{S} \rangle \to \langle \mathcal{E}, \mathcal{S} \rangle\)
  that maps formation, universe, and state to an expression and a new state.
\end{definition}
...
is a possibly empty ordered set of key-value pairs, where all keys are unique.
\end{definition}

\subsection{Formation}\label{sec:formation}

\begin{definition}[Formation]
A \textbf{formation}, denoted as $[[ B ]]$, ranged over \(\mathcal{B}\) by \(b_i\), is a binding.
\end{definition}
...
\subsection{Parent}\label{sec:parent}
