...
The purpose of this section is to define the entities that constitute the calculus---such as
  expressions, formations, attributes, bindings, and scope---together with their structural relationships.
...
is a binding where keys are either attributes or assets and values are either $?$, expressions, data, or functions.
\end{definition}
...
A \textbf{domain} of formation \(b\), denoted as \(\bar{b}\), is a set that includes all attributes of \(b\).
\end{definition}
...
\subsection{Abstract Formation}\label{sec:abstract}

\begin{definition}[Abstract]
An formation is \textbf{abstract} if at least one of its attributes is void;
otherwise the formation is \textbf{closed}.
\end{definition}

\Cref{eq:price-color} is an example of a formation,
  where the binding of the formation being formed consists of two pairs:
  $price -> ?$ and $color -> [[ D> FF-C0-CB ]]$.
The expression attached to the |color| attribute is a closed formation.
...
An \textbf{application}, denoted as $ b ( \tau -> e ) $,
  is a copy of an abstract formation \(b\) (the ``subject''),
  with \(e\) attached to its \(\tau\) attribute.
\end{definition}

\Cref{eq:simple-application} demonstrates application,
  where $ |a| -> b_2 $ is applied to the abstract formation $[[ |a| -> ? ]]$.
The application creates a new formation $[[ |a| -> b_2 ]]$,
  while the existing formation remains intact.

The formation in \cref{eq:price-color} is abstract, because its attribute \ff{price} is void.
The formation in \cref{eq:simple-application} was abstract before the application,
...
In $b(\phiTerminal{\alpha_i} -> e)$ application, \(e\) must be attached to the attribute \(\tau\) of \(b\)
  for which \(\ordinal{\tau}{\mathbb{F}(b)}\) equals \(i\).

\subsection{Forma}\label{sec:forma}

\begin{definition}[Forma]
A \textbf{forma} of a formation \(b\), denoted as \(\mathbb{F}(b)\), is an abstract formation
  that was copied in order to create \(b\).
\end{definition}

In \cref{eq:simple-application}, the forma of $[[ |a| -> b_2 ]]$ is the abstract
  formation $[[ |a| -> ? ]]$, while the forma of $[[ |a| -> ? ]]$ is itself.

\subsection{Dispatch}\label{sec:dispatch}

\begin{definition}[Dispatch]
A \textbf{dispatch}, denoted as $ b . \tau $, where \(b\) is the \enquote{subject,}
  means retrieval of expression attached to the attribute \(\tau\) of \(b\).
\end{definition}
...
In simpler terms, the scope is the formation that is the \enquote{closest} to the pair, moving to the left in the expression.
...
The \textbf{terminator}, denoted as $T$, is an formation that equals itself when it is the subject of a dispatch or an application:
\begin{phiquation*}
\forall \tau : T.\tau \trans T \qquad \forall \tau {,}\; e : T ( \tau -> e ) \trans T.
\end{phiquation*}
\end{definition}
...
Every formation is \textbf{immutable}, meaning that an application of its already attached attribute equals $T$:
\begin{phiquation*}
\forall \tau {,}\; e_1 {,}\; e_2 : [[ \tau -> e_1 ]] ( \tau -> e_2 ) \trans T
\end{phiquation*}
\end{definition}
...
Consider the following formation of an abstract formation with an inner formations:
\begin{phiquation*}
