...
Inspection of the rules in \cref{fig:reduction} shows that their left-hand sides are
  distinguished by mutually exclusive syntactic and semantic conditions: for example,
    rules \nameref{r:dot} and \nameref{r:null} require the same attribute to be attached to a normal form or to \(\varnothing\), respectively;
    rules \nameref{r:phi} and \nameref{r:stop} differ by the presence of \(\varphi\);
...
In rule \nameref{r:copy}, the replaced expression is the normal form of a strictly smaller expression \(\ctx{e}{e\textsuperscript{\(\varsigma\)}}\).
Any alternative reduction path that first reduces inside this subexpression must converge to the same normal form,
  since normal forms of smaller expressions are unique by induction on expression size.
...
All remaining rules either erase subexpressions or reroute dispatch and application without depending on the evaluation of nested redexes;
in such cases the inner and outer reductions are either disjoint or one excludes the other.

Thus, in all cases, the two one-step reductions are joinable.
\end{proof}
...
Suppose \(e \strans n_1\) and \(e \strans n_2\).
By confluence, there exists \(u\) such that \(n_1 \strans u\) and \(n_2 \strans u\).
Since \(n_1\) and \(n_2\) are irreducible, we have \(n_1 = u = n_2\).
Therefore, normal forms are unique.
\end{proof}
