...
Dataizing the program (\cref{sec:sem:atoms}) reduces the dispatch chain
  through \ff{stdout}'s decoratee (a sequence that calls
  \ff{console.write} on the supplied text), and ultimately reaches the
  atom that writes the greeting bytes to the host's standard output.
The atom that touches the host lives inside the \ff{console} object,
  not inside \ff{stdout} itself --- \ff{stdout} is a regular
  formation that decorates a \ff{console}-based output sequence.
...
\subsection{Bindings in a Reversed Dispatch}\label{app:ex:binding-chain}

Inline bindings (\ff{:label}, \ff{:N}) on a horizontal application group
  are governed by two rules of \cref{sec:wf:bindings}:
  the all-or-nothing rule (R-6.6.2) and the receiver-exclusion rule
  (R-6.6.3).
A reversed dispatch line exercises both at once: its first horizontal
  argument is the receiver, the remaining horizontal arguments form an
  ordinary argument group.
The receiver cannot carry a binding;
  the rest of the arguments must be uniformly bound or uniformly
  unbound.

\begin{ffcode}
if. true x y              // legal: all unbound, receiver `true' bare
if. true x:a y:b          // legal: receiver `true' bare, args all bound
if. true:a x y            // rejected: receiver cannot carry a binding
if. true x:a y            // rejected: arg group mixes bound and unbound
\end{ffcode}

The same rules apply to any horizontal application:
  \ff{foo a:1 b:2} is legal (all bound), \ff{foo a b} is legal (all
  unbound), and \ff{foo a:1 b} is rejected (mixed).
