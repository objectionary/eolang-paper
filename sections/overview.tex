...
The \ff{color} attribute is attached to another object formation with one $D$-\emph{asset},
...
An object may have data attached only to its $D$-asset, for example:
\begin{phiquation*}
[[ |x| -> [[ D> 00-2A ]] ]] {.}
\end{phiquation*}

An object may have a \emph{function} attached to its $L$-asset.
...
There is also a \emph{dataization} function \(\mathbb{D}(b, u, s)\)
  ...
  attached to the $D$-asset of the normal form or morphs the atom into an object and dataizes it (recursively),
  for example:
\begin{phiquation*}
\mathbb{D} {(} [[ |x| -> 42 ]].|x| \trans 42 {,}\; u {,}\; s {)} \to 42.
\end{phiquation*}

A program is an object, called \emph{Universe}, that is attached to the $Q$ attribute of nowhere:
\begin{phiquation*}
Q -> [[ D> CA-FE ]].
\end{phiquation*}
