...
An \emph{formation} is a collection of \emph{attributes}, which are uniquely named pairs, for example:
\begin{phiquation}
\label{eq:price-color}
[[ price -> ?, color -> [[ D> FF-C0-CB ]] ]] {.}
\end{phiquation}
This formation has two attributes \ff{price} and \ff{color}.
The formation is \emph{abstract} because the \ff{price} attribute is \emph{void}, i.e. there is nothing \emph{attached} to it.
The \ff{color} attribute is attached to another formation with one $D$-\emph{asset},
  which is attached to \emph{data} (three bytes).
Formations do not have names; only attributes do.

A \emph{full application} of an abstract formation to a pair of an attribute and an \emph{expression}
...
An formation may be \emph{dispatched} from another formation with the help of \emph{dot notation}
...
The leftmost symbol \phiTerminal{\(\xi\)} denotes the \emph{scope} of the formation,
  ...
  the following \(.\ff{p}\) part retrieves the formation attached to the attribute \(\ff{p}\);
  then, the application $(|t|->b)$ makes a copy of the formation;
  finally, the \(.\ff{y}\) part retrieves formation \(b\).

Each formation has a \emph{parent} attribute $^$, which is attached to the
  formation from which the formation was dispatched.
...
Every expression has a \emph{normal form}, which is a form that has no further applicable reductions.

An formation is called a \emph{decorator} if it has the $@$-attribute with an formation attached to it,
...
An formation may have data attached only to its $D$-asset, for example:
\begin{phiquation*}
[[ |x| -> [[ D> 00-2A ]] ]] {.}
\end{phiquation*}

An formation may have a \emph{function} attached to its $L$-asset.
Such an formation is referred to as an \emph{atom}.
An atom may be \emph{morphed} to another formation by evaluating its function with
\begin{inparaenum}[1)]
  \item the formation and
